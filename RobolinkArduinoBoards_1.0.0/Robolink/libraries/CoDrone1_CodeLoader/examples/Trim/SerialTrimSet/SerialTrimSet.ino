/*****************************************************************
  ...
  모든 값은 -500 ~ 500으로 설정하며, 초기 기준 값은 0입니다.
  ...
  if (Serial.available() > 0)
  {
    ...
    else
    {
      if (cmdBuff[cmdIndex - 1] == '\n')
      {
        ...
        else if (strcmp(cmd, "now") == 0)     PrintTrimAll();
...
void PrintMenu()
{
  ...
  Serial.println("- Serial Monitor : Newline, 115200 baud");
...
void PrintTrimAll()
{
  ...
  Serial.print("roll \t\t");  Serial.println(trim.roll);
  Serial.print("pitch \t\t");  Serial.println(trim.pitch);
  Serial.print("yaw \t\t");  Serial.println(trim.yaw);
  Serial.print("throttle \t");  Serial.println(trim.throttle);
  Serial.println("--------------------------------------------- ");
